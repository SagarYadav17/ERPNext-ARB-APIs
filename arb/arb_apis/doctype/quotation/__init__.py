...
from frappe.utils import nowdate, now_datetime, getdate, add_days, flt, cint
...
def get_quotations(filters=None, page=1, page_size=20):
    try:
        ...
        quotations = frappe.get_all(
            "Quotation",
            fields=[
                "name", "party_name", "contact_email",
                "transaction_date", "valid_till",
                "net_total", "grand_total", "rounded_total",
                "status", "company"
            ],
            start=start,
...
def record_payment(quotation_id, payment_method, amount=None, transaction_id=None, notes=None):
    ...
    try:
        ...
        if quotation.status != "Ordered":
            return {
                "success": False,
                "error": "Only approved quotations can be paid"
            }
        ...
        # Add comment
        quotation.add_comment("Comment", f"Payment received via {payment_method}. Amount: {amount or quotation.grand_total}. Transaction ID: {transaction_id}")
...
def edit_quotation():
    try:
        ...
        update_fields = ['customer_name', 'customer_email', 'customer_phone', 
                        'customer_company', 'customer_gst', 'notes', 'terms', 'valid_until']
...
def get_react_status(backend_status):
    ...
    return status_map.get(backend_status, 'draft')
...
def get_tax_template(company):
    ...
    template = frappe.db.get_value("Sales Taxes and Charges Template", 
                                  {"company": company, "title": "GST"}, "name")
    return template
